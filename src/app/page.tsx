...
export default function Home() {
  ...
  return (
    ...
    <main className="app-shell">
      ...
      {mobileMoreOpen && <div className="mobile-more-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setMobileMoreOpen(false); }}><section className="mobile-more-panel" role="dialog" aria-modal="true" aria-labelledby="mobile-more-title"><div className="mobile-more-head"><h2 id="mobile-more-title">Workspace</h2><button className="dialog-close" aria-label="Close workspace menu" onClick={() => setMobileMoreOpen(false)}>×</button></div><div className="mobile-more-grid">{["Knowledge", "Projects", "Decisions", "Milestones", "Ideas"].map((item) => <button key={item} className={active === item ? "selected" : ""} onClick={() => { setActive(item); setQuery(""); setMobileMoreOpen(false); }}><span>{icons[item]}</span>{item}</button>)}</div></section></div>}

      <section className="main-column">
        <header className="topbar"><div className="breadcrumb"><span>MYOS</span><span className="crumb-slash">/</span><strong>{active}</strong></div><div className="topbar-right"><span className="today-label">Your personal archive</span>{user && <button className="top-signout" onClick={() => void getSupabase()?.auth.signOut()}>Sign out</button>}<button className="top-capture" onClick={() => openNewCapture()}>＋ <span>Capture</span></button></div></header>
