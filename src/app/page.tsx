...
const captureTypes: CaptureType[] = ["Capture", "Knowledge", "Idea", "Project", "Decision", "Milestone", "Goal", "Journal", "Book", "Resource", "Media", "Task", "Person"];
const navigation = ["Home", "Timeline", "Knowledge", "Projects", "Decisions", "Milestones", "Ideas", "Media"];
...
const icons: Record<string, string> = { Home: "⌂", Timeline: "◷", Knowledge: "▤", Projects: "▱", Decisions: "◇", Milestones: "✳", Ideas: "✧", Media: "▣" };

function captureTypeLabel(type: CaptureType): string {
  return type === "Media" ? "Web series / Movies" : type;
}

function navigationLabel(item: string): string {
...
export default function Home() {
  ...
  const visibleCaptures = useMemo(() => captures
    .filter((item) => {
      ...
      const typeForSection: Record<string, CaptureType> = { Knowledge: "Knowledge", Projects: "Project", Decisions: "Decision", Milestones: "Milestone", Ideas: "Idea", Media: "Media" };
  ...
  function handleCommandKey(event: KeyboardEvent<HTMLInputElement>) {
    if (event.key === "Enter") {
      ...
      const match = captureTypes.find((candidate) => candidate.toLowerCase() === command.toLowerCase()
        || captureTypeLabel(candidate).toLowerCase() === command.toLowerCase());
  ...
  return (
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="primary-nav" aria-label="Primary navigation">{navigation.map((item) => <button key={item} className={`nav-item ${active === item ? "selected" : ""}`} onClick={() => { if (item === "Timeline") { showTimeline(); return; } setActive(item); setQuery(""); }}><span className="nav-icon">{icons[item]}</span>{navigationLabel(item)}{item === "Knowledge" && <span className="nav-count">{captures.filter((capture) => capture.type === "Knowledge").length}</span>}</button>)}</nav>
        ...
        <div className="sidebar-bottom"><div className="privacy-note"><span className="privacy-dot"/><span>{user ? "Synced to your account" : "Saved on this device"}<br/><small>{user?.email ?? "Not synced or backed up."}</small></span></div><div className="profile-button"><span className="avatar">m</span><span className="profile-name">{user?.email ?? "My workspace"}<small>{user ? "Private cloud sync" : "Local storage"}</small></span></div></div>
      </aside>

      {mobileMoreOpen && <div className="mobile-more-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setMobileMoreOpen(false); }}><section className="mobile-more-panel" role="dialog" aria-modal="true" aria-labelledby="mobile-more-title"><div className="mobile-more-head"><h2 id="mobile-more-title">Workspace</h2><button className="dialog-close" aria-label="Close workspace menu" onClick={() => setMobileMoreOpen(false)}>×</button></div><div className="mobile-more-grid">{["Knowledge", "Projects", "Decisions", "Milestones", "Ideas", "Media"].map((item) => <button key={item} className={active === item ? "selected" : ""} onClick={() => { setActive(item); setQuery(""); setMobileMoreOpen(false); }}><span>{icons[item]}</span>{navigationLabel(item)}</button>)}</div></section></div>}

      <section className="main-column">
        <header className="topbar"><div className="breadcrumb"><span>MYOS</span><span className="crumb-slash">/</span><strong>{navigationLabel(active)}</strong></div><div className="topbar-right"><span className="today-label">Your personal archive</span>{user && <button className="top-signout" onClick={() => void getSupabase()?.auth.signOut()}>Sign out</button>}<button className="top-capture" onClick={() => openNewCapture()}>＋ <span>Capture</span></button></div></header>
        <div className="page-content">
          <section className="welcome"><div><div className="eyebrow"><span className="eyebrow-line"/> YOUR PERSONAL SPACE</div><h1>{active === "Home" ? captures.length ? "A little more clarity." : "Start with a thought." : navigationLabel(active)}</h1><p className="welcome-copy">{active === "Home" ? "Your thoughts, decisions, and progress — all in one place." : active === "Timeline" ? "A quiet record of what you have learned, made, and lived." : `Everything you have collected in ${navigationLabel(active).toLowerCase()}.`}</p></div><div className="welcome-date"><span className="date-day">MY</span><span className="date-month">PERSONAL<br/>ARCHIVE</span></div></section>
          ...
          <div className="section-heading"><div><div className="section-kicker">{active === "Timeline" ? "PERSONAL HISTORY" : ready ? `${captures.length} SAVED ${captures.length === 1 ? "ITEM" : "ITEMS"}` : "LOADING YOUR SPACE"}</div><h2>{query ? "Search results" : active === "Timeline" ? timelineRangeLabel(timelineDate || localDateValue(new Date()), timelineView) : active === "Home" ? "Recent captures" : navigationLabel(active)}</h2></div>{active === "Timeline" ? <div className="timeline-controls"><button className="timeline-nav-button" aria-label="Previous period" onClick={() => moveTimeline(-1)}>‹</button><input aria-label="Jump to date" type="date" value={timelineDate || localDateValue(new Date())} onChange={(event) => setTimelineDate(event.target.value)}/><button className="timeline-nav-button" aria-label="Next period" onClick={() => moveTimeline(1)}>›</button><select aria-label="Timeline view" value={timelineView} onChange={(event) => setTimelineView(event.target.value as TimelineView)}><option value="year">Year</option><option value="month">Month</option><option value="day">Day</option></select><select aria-label="Filter captures by type" value={timelineType} onChange={(event) => setTimelineType(event.target.value)}><option>All types</option>{captureTypes.map((captureType) => <option key={captureType} value={captureType}>{captureTypeLabel(captureType)}</option>)}</select><button className="timeline-today" onClick={() => setTimelineDate(localDateValue(new Date()))}>Today</button></div> : active === "Projects" ? <button className="date-link" onClick={() => openNewCapture("Project")}>＋ New project</button> : <button className="date-link" onClick={showTimeline}>Timeline <span>→</span></button>}</div>
      ...
      <nav className="mobile-nav" aria-label="Mobile navigation"><button aria-current={active === "Home" ? "page" : undefined} className={active === "Home" ? "selected" : ""} onClick={() => { setActive("Home"); setQuery(""); setMobileMoreOpen(false); }}><span className="mobile-nav-icon">⌂</span><span>Home</span></button><button aria-current={active === "Timeline" ? "page" : undefined} className={active === "Timeline" ? "selected" : ""} onClick={showTimeline}><span className="mobile-nav-icon">◷</span><span>Timeline</span></button><button className="mobile-nav-capture" onClick={() => { setMobileMoreOpen(false); openNewCapture(); }}><span className="mobile-capture-plus">＋</span><span>Capture</span></button><button aria-current={active === "Search" ? "page" : undefined} className={active === "Search" ? "selected" : ""} onClick={() => { setActive("Search"); setQuery(""); setMobileMoreOpen(false); window.setTimeout(() => document.getElementById("global-search")?.focus(), 0); }}><span className="mobile-nav-icon"><SearchIcon /></span><span>Search</span></button><button aria-expanded={mobileMoreOpen} className={mobileMoreOpen || ["Knowledge", "Projects", "Decisions", "Milestones", "Ideas", "Media"].includes(active) ? "selected" : ""} onClick={() => setMobileMoreOpen((open) => !open)}><span className="mobile-nav-icon">···</span><span>More</span></button></nav>
